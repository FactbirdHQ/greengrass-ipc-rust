...
pub fn create_publish_iot_request(topic: &str, payload_data: &[u8]) -> EventStreamMessage {
    ...
    let payload = format!(r#"{{
        ...
        "qos": "AT_MOST_ONCE",
        "payload": "{}"
    }}"#, topic, base64::engine::general_purpose::STANDARD.encode(payload_data));
...
pub fn create_get_shadow_request(thing_name: &str, shadow_name: Option<&str>) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload
...
pub fn create_update_shadow_request(thing_name: &str, shadow_name: Option<&str>, state_json: &str) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload
...
pub fn create_delete_shadow_request(thing_name: &str, shadow_name: Option<&str>) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload
    let payload = if let Some(shadow_name) = shadow_name {
        format!(r#"{{
            "thingName": "{}",
...
pub fn create_get_configuration_request(component_name: &str, key_path: &[&str]) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload with key path as array
    let key_path_json = key_path.iter()
        .map(|k| format!("\"{}\"", k))
        .collect::<Vec<_>>()
        .join(", ");
...
pub fn create_subscribe_configuration_update_request(component_name: &str, key_path: &[&str]) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload with key path as array
    let key_path_json = key_path.iter()
        .map(|k| format!("\"{}\"", k))
        .collect::<Vec<_>>()
...
pub fn create_subscribe_validate_configuration_updates_request(component_name: &str) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload
    let payload = format!(r#"{{
        "componentName": "{}"
    }}"#, component_name);
...
pub fn create_update_configuration_request(component_name: &str, key_path: &[&str], value_json: &str) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload with key path as array
    let key_path_json = key_path.iter()
        .map(|k| format!("\"{}\"", k))
        .collect::<Vec<_>>()
...
pub fn create_configuration_validity_report_request(status: &str, deployment_id: &str) -> EventStreamMessage {
    // For testing purposes, we'll create a simple JSON payload
    let payload = format!(r#"{{
