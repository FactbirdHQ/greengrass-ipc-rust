...
async fn compare_message(
    harness: &MessageComparisonHarness,
    message: &greengrass_ipc_rust::event_stream::EventStreamMessage,
    operation: &str,
) {
    // Encode the message
    ...
    println!(
        "Message comparison result for {}: matches={}",
...
async fn test_configuration_operations_encoding() {
    ...
    let get_config = create_get_configuration_request("com.example.TestComponent", key_path);
    compare_message(&harness, &get_config, "GetConfiguration").await;

    // Test SubscribeToConfigurationUpdate
    let sub_config =
        create_subscribe_configuration_update_request("com.example.TestComponent", key_path);
    compare_message(&harness, &sub_config, "SubscribeToConfigurationUpdate").await;

    // Test SubscribeToValidateConfigurationUpdates
    let sub_validate = create_subscribe_validate_configuration_updates_request();
    compare_message(
        &harness,
        &sub_validate,
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .as_millis() as u64;
    let update_config =
        create_update_configuration_request(key_path, r#"{"new_value": 123}"#, timestamp);
    compare_message(&harness, &update_config, "UpdateConfiguration").await;

    // Test SendConfigurationValidityReport
    let validity_report =
        create_configuration_validity_report_request("ACCEPTED", "deployment-123");
    compare_message(
        &harness,
        &validity_report,
...
mod message_structure_tests {
    ...
    async fn test_decode_python_messages() {
        ...
        for message in python_messages {
            println!(
                "Python message: operation={}, type={}, size={}",
                message.operation,
    ...
    fn test_event_stream_message_structure() {
        ...
        assert!(
            encoded.len() >= 16,
            "Message too short, missing required fields"
        );
