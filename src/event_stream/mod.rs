...
impl EventStreamMessage {
    ...
    pub fn encode(&self) -> Result<Bytes> {
        ...
        let total_len = PRELUDE_LENGTH + PRELUDE_CRC_LENGTH + headers_len + payload_len + TRAILER_LENGTH;
    ...
    fn encode_header(&self, name: &str, value: &HeaderValue) -> Result<Bytes> {
        ...
        match value {
            HeaderValue::Bool(val) => {
                buffer.put_u8(0); // type code for bool
                buffer.put_u8(if *val { 1 } else { 0 });
            },
            HeaderValue::I8(val) => {
                buffer.put_u8(1); // type code for i8
                buffer.put_i8(*val);
            },
            HeaderValue::U8(val) => {
                buffer.put_u8(2); // type code for u8
                buffer.put_u8(*val);
            },
            HeaderValue::I16(val) => {
                buffer.put_u8(3); // type code for i16
                buffer.put_i16(*val);
            },
            HeaderValue::U16(val) => {
                buffer.put_u8(4); // type code for u16
                buffer.put_u16(*val);
            },
            HeaderValue::I32(val) => {
                buffer.put_u8(5); // type code for i32
                buffer.put_i32(*val);
            },
            HeaderValue::U32(val) => {
                buffer.put_u8(6); // type code for u32
                buffer.put_u32(*val);
            },
            HeaderValue::I64(val) => {
                buffer.put_u8(7); // type code for i64
                buffer.put_i64(*val);
            },
            HeaderValue::U64(val) => {
                buffer.put_u8(8); // type code for u64
                buffer.put_u64(*val);
            },
            HeaderValue::String(val) => {
                buffer.put_u8(9); // type code for String
            ...
            HeaderValue::Bytes(val) => {
                buffer.put_u8(10); // type code for Bytes
            ...
            HeaderValue::Timestamp(val) => {
                buffer.put_u8(11); // type code for Timestamp
                buffer.put_u64(*val);
            },
            HeaderValue::Uuid(val) => {
                buffer.put_u8(12); // type code for UUID
                buffer.extend_from_slice(val);
            },
        }
    ...
    fn decode_header(cursor: &mut Cursor<&[u8]>) -> Result<(String, HeaderValue)> {
        ...
        let value = match header_type {
            0 => HeaderValue::Bool(cursor.get_u8() != 0),
            1 => HeaderValue::I8(cursor.get_i8()),
            2 => HeaderValue::U8(cursor.get_u8()),
            3 => HeaderValue::I16(cursor.get_i16()),
            4 => HeaderValue::U16(cursor.get_u16()),
            5 => HeaderValue::I32(cursor.get_i32()),
            6 => HeaderValue::U32(cursor.get_u32()),
            7 => HeaderValue::I64(cursor.get_i64()),
            8 => HeaderValue::U64(cursor.get_u64()),
            9 => {
            ...
            11 => HeaderValue::Timestamp(cursor.get_u64()),
            12 => {
                let mut uuid = [0; 16];
                cursor.read_exact(&mut uuid).map_err(Error::from)?;
                HeaderValue::Uuid(uuid)
            },
            _ => return Err(Error::InvalidInput(format!("Unknown header type: {}", header_type))),
        };
